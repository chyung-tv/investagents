...
- Paper fills need a last price. Without `FINANCIAL_DATASETS_API_KEY`, set web `PORTFOLIO_QUOTE_STUB=MSFT:400` so settle can demo a buy/sell. Worker boots and polls with placeholder keys (`uv run python -m research_team.worker --once` → "no due jobs"), but a real tick needs live `OPENROUTER_API_KEY`, `EXA_API_KEY`, and `FINANCIAL_DATASETS_API_KEY`.
