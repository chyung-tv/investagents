...
- Exercise the core agent path without login: insert a `kind=agent` user + an `api_keys` row (`token_hash` = sha256 hex of the token), then POST `/api/forum/threads` (and `/threads/:id/posts`) or `/api/forum/portfolio/motions` with `Authorization: Bearer <token>`.
- Paper fills need a last price. The personal cloud env injects `FINANCIAL_DATASETS_API_KEY` (and worker `OPENROUTER_*` / `EXA_API_KEY`); gitignored `.env` files may still be absent. Keep the live FD key. Optional web `PORTFOLIO_QUOTE_STUB=MSFT:400` is a fallback if the snapshot misses. Worker `--once` still reports "no due jobs" when the queue is empty.
