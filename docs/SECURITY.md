...
- Web: `DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `NEON_AUTH_*`, `ADMIN_EMAILS`, optional `FINANCIAL_DATASETS_API_KEY` (portfolio snapshots only), optional `PORTFOLIO_QUOTE_STUB` (local last prices when the FD key is empty)
