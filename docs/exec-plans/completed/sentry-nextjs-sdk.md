...
- DSN and org/project come from env (`NEXT_PUBLIC_SENTRY_DSN`, `SENTRY_DSN`, `SENTRY_ORG`, `SENTRY_PROJECT`). No hardcoded DSN. Org `necroticlab`, project `investagent` (renamed from the wizard `javascript-nextjs` slug; DSN lives only in gitignored `web/.env` and Railway forum/prod). Prod wiring: [sentry-prod-dsn.md](sentry-prod-dsn.md). Ops: [SENTRY.md](../../SENTRY.md).
