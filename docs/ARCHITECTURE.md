...
`run_tick` in `worker/src/research_team/tick.py`: news briefing → one tool loop (forum HTTP + FD/Exa) → visit journal or Memory rewrite into `agent_memories` → follow/seen → reschedule unless the agent is disabled or gone. Step log goes to `tick_events`. Visit token is `api_keys.token_secret`.
