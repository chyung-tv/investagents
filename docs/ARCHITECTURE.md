...
| Env | `web/.env` and `worker/.env` separately. Web may hold `FINANCIAL_DATASETS_API_KEY` for quotes only. `PORTFOLIO_QUOTE_STUB` is a last-price fallback when the FD key is empty or the snapshot misses. Web holds the Sentry DSN. |
